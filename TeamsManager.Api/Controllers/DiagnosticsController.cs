...
namespace TeamsManager.Api.Controllers
{
    ...
    public class DiagnosticsController : ControllerBase
    {
        ...
        public async Task<ActionResult<object>> GetGraphStatusAsync()
        {
            try
            {
                _logger.LogInformation("Pobieranie statusu Graph API");

                var healthTask = _graphConnectionService.GetConnectionHealthAsync();
                var diagnosticTask = _graphConnectionService.GetDiagnosticInfoAsync();
                var permissionTask = _graphConnectionService.GetPermissionInfoAsync();
                var rateLimitTask = _graphConnectionService.GetRateLimitStatusAsync();

                await Task.WhenAll(healthTask, diagnosticTask, permissionTask, rateLimitTask);

                var healthInfo = await healthTask;
                var diagnostic = await diagnosticTask;
                var permissionInfo = await permissionTask;
                var rateLimitStatus = await rateLimitTask;
                ...
                var graphStatus = new
                {
                    ...
                    Authentication = new
                    {
                        IsAuthenticated = diagnostic.IsAuthenticated,
                        TenantId = diagnostic.TenantId,
                        ApplicationId = diagnostic.ApplicationId,
                        GraphApiVersion = diagnostic.GraphApiVersion ?? "v1.0"
                    },
                    Permissions = new
                    {
                        HasRequiredPermissions = diagnostic.HasRequiredPermissions,
                    ...
                    RateLimit = new
                    {
                        ...
                        IsLimitReached = rateLimitStatus?.IsLimitReached == true
                    },
                    Diagnostics = new
                    {
                        AllTestsPassed = diagnostic.AllTestsPassed,
                        ErrorCount = diagnostic.Errors.Count,
                        WarningCount = diagnostic.Warnings?.Count ?? 0,
                        LastDiagnosticRun = diagnostic.LastChecked
                    },
                    Issues = diagnostic.Errors.Count > 0 ? diagnostic.Errors : new List<string>(),
                    Warnings = diagnostic.Warnings?.Count > 0 ? diagnostic.Warnings : new List<string>()
                };

                _logger.LogInformation("Status Graph API pobrany. Status: {OverallStatus}, Połączony: {IsConnected}",
                    graphStatus.OverallStatus, graphStatus.Connection.IsConnected);

                return Ok(graphStatus);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Błąd podczas pobierania statusu Graph API");
                return StatusCode(500, new { error = "Błąd podczas pobierania statusu Graph API", details = ex.Message });
            }
        }
