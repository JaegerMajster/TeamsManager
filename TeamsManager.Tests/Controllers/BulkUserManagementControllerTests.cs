...
namespace TeamsManager.Tests.Controllers
{
    ...
    public class BulkUserManagementControllerTests
    {
        ...
        public async Task CancelProcess_WithValidProcessId_ShouldReturnOk()
        {
            ...
            result.Should().BeOfType<OkObjectResult>();
            var okResult = result as OkObjectResult;
            var response = okResult!.Value.Should().BeAssignableTo<CancelProcessResponse>().Subject;

            response.Success.Should().BeTrue();
            response.Message.Should().Be($"Proces {processId} został anulowany");
        }
        ...
        public async Task CancelProcess_WithNonExistentProcessId_ShouldReturnNotFound()
        {
            ...
            var response = notFoundResult!.Value.Should().BeAssignableTo<CancelProcessResponse>().Subject;

            response.Success.Should().BeFalse();
            response.Message.Should().Be($"Proces {processId} nie istnieje lub nie może być anulowany");
        }
        ...
        public async Task CancelProcess_WhenOrchestratorThrowsException_ShouldReturnInternalServerError()
        {
            ...
            var objectResult = result as ObjectResult;
            objectResult!.StatusCode.Should().Be(500);
            objectResult.Value.Should().Be("Wystąpił błąd podczas anulowania procesu");
        }
