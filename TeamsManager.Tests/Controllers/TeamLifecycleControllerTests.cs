...
namespace TeamsManager.Tests.Controllers
{
    ...
    public class TeamLifecycleControllerTests
    {
        ...
        public async Task CancelProcess_WithEmptyProcessId_ShouldReturnBadRequest()
        {
            ...
            // Assert
            var badRequestResult = result.Should().BeOfType<BadRequestObjectResult>().Subject;
            var response = badRequestResult.Value.Should().BeOfType<CancelProcessResponse>().Subject;
            response.Success.Should().BeFalse();
            response.Message.Should().Be("ID procesu jest wymagane");
        }

        [Fact]
        public async Task CancelProcess_WithException_ShouldReturnInternalServerError()
        {
            // Arrange
            var processId = "process-123";
